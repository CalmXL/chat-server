...
- [x] 实现 `RefreshTokenDto`，严格校验 `refreshToken` 字段格式。
- [x] 实现 `POST /api/v1/auth/refresh` 接口，验证 Refresh Token 签名的有效性。
- [x] 校验 Refresh Token 中的 `jti` 是否与 Redis 中记录的当前活跃 `refreshTokenJti` 一致：
...
- [x] 编写令牌轮转端到端（E2E）测试，覆盖合法 Token 正常换发新 Token 对、轮转后旧 Token 再次使用触发重放防御拦截并清空会话的完整场景。
